...
public interface FactPlantStatusRepository extends JpaRepository<FactPlantStatus, FactPlantStatusId> {
    @Query("SELECT fs.measurementValue FROM FactPlantStatus fs WHERE fs.plantID.plantID = (:id) and fs.measurementType = (:measurement)")
    List<Double> getMeasurementHistory(String measurement, int id);

    @Query("SELECT AVG(fs.measurementValue) FROM FactPlantStatus fs WHERE fs.plantID.plantID = (:id) and fs.measurementType = (:measurement)")
    double getAverageMeasurement(String measurement, int id);

    @Query("SELECT fs FROM FactPlantStatus fs WHERE fs.plantID.plantID = (:id) and fs.measurementType = (:measurement)")
    List<FactPlantStatus> getMeasurementHistoryForPlant(String measurement, int id);

    FactPlantStatus findFirstByMeasurementTypeAndPlantID(String measurement_type, DimPlant plant_ID);
